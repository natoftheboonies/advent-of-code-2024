...
def top_and_tail(data, block_size=5, include_line_numbers=True, zero_indexed=False):
    ...
    if isinstance(data, list):
        ...
        if len(data) < 11:
            return "\n".join(
                format_with_line_number(i, line) for i, line in enumerate(data)
            )
        else:
            top = [
                format_with_line_number(i, line)
                for i, line in enumerate(data[:block_size])
            ]
            tail = [
                format_with_line_number(i, line)
                for i, line in enumerate(
                    data[-block_size:], start=len(data) - block_size
                )
            ]
            return "\n".join(top + ["..."] + tail)
    else:
...
def get_locations(script_file) -> Locations:
    ...
    return Locations(
        script_name, script_dir, input_dir, output_dir, sample_input_file, input_file
    )
...
def write_puzzle_input_file(year: int, day: int, locations: Locations) -> str:
    ...
    logger.info(
        "Session cookie retrieved: %s...%s", session_cookie[0:6], session_cookie[-6:]
    )
    ...
    if response.status_code == 200:
        ...
        with open(locations.input_file, "w") as file:
            logger.debug(
                "Writing input file %s", os.path.basename(locations.input_file)
            )
            file.write(data)
            return data
    else:
        raise ValueError(
            f"Unable to retrieve input data. HTTP response: {response.status_code}"
        )
